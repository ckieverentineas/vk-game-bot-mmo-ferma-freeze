...
export async function User_Register(context: Context) {
    ...
    await context.send(`✅ Ваш бизнес план на старте:\n\n1 Нажмите кнопку Здания и постройте свое первое здание;\n2 Нажмите кнопку Люди и наймите первого рабочего;\n3 Нажмите кнопку Люди и затем кнопку 👣 для выбора места работы нанятому рабочему;\n4 Нажмите кнопку Прибыль, чтобы запустить производство, а через 30-60 секунд нажмите по этой кнопке еще раз;\n5 После накопления 10 энергии можете смело жать кнопку Биржа для конвертации энергии в шекели.`)
