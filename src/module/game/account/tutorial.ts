...
export async function User_Register(context: Context) {
    ...
    let name = null
    while (!name) {
        const name_in = await context.question( `🧷 Через некоторое время ты вкатился в систему под своей учеткой 🏦! Похоже, ты здесь новенький. Назови свое имя и фамилию, малец.\n❗ Учти! Ложные данные — это серьезное дело, и за это могут настучать по шапке!`, timer_text)
